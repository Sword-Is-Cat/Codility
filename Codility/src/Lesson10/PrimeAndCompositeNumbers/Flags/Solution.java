...
class Solution {
	public int solution(int[] A) {
		...
		int first = peakList.get(0);
		int end = peakList.get(peakCnt - 1);

		int distance = end - first;

		//깃발수 flagCnt 에 대해 flagCnt(flagCnt-1) <= distance 이므로 역산하여 flagCnt의 최대값을 구한다
		//n^2 - n <= d  ->  n^2 -n +0.25 (= (n-0.5)^2 ) <= d + 0.25  ->  n-0.5 <= sqrt(d+0.25)  -->  n <= sqrt(d+0.25)+0.5 
		int flagCnt = (int) (Math.sqrt(distance + 0.25D) + 0.5D);

		//깃발수의 최대값부터 감소하며 해당 갯수의 깃발을 꼽을 수 있는지 확인한다
		for (int i = flagCnt; i > 0; i--) {
			queue.clear();
			queue.addAll(peakList);
			int result = vaildCnt(queue, i);
			if (result > 0)
				return result;
		}
